...
if (isset($_GET['add']) || isset($_GET['edit'])) {
	...
	$shortdescription = ((isset($_POST['shortdescription']) && $_POST['shortdescription'] != '') ? sanitize($_POST['shortdescription']) : '');
	...
	if (isset($_GET['edit'])) {
		...
		$shortdescription = ((isset($_POST['shortdescription']) && !empty($_POST['shortdescription'])) ? sanitize($_POST['shortdescription']) : $product['shortdescription']);
	...
	if ($_POST) {
		...
		if (!empty($errors)) {
			...
			$insertSql = "INSERT INTO products (`title`,`price`,`list_price`,`brand`,`categories`,`sizes`,`image`,`description`,`shortdescription`) 
        VALUES ('$title', '$price', '$list_price', '$brand', '$category', '$sizes', '$dbpath', '$description', '$shortdescription')";
			if (isset($_GET['edit'])) {
        ...
        shortdescription = '$shortdescription' WHERE id = '$edit_id'";
			}
  ...
  <form action="products.php?<?= ((isset($_GET['edit'])) ? 'edit=' . $edit_id : 'add=1'); ?>" method="post"
      ...
      <select name="brand" id="brand" class="form-control">
        <option value=""<?= (($brand == '') ? ' selected' : ''); ?>></option>
		  <?php while ($b = mysqli_fetch_assoc($brandQuery)) : ?>
            <option value="<?= $b['id']; ?>"<?= (($brand == $b['id']) ?
				' selected' : ''); ?>><?= $b['brand']; ?></option>
		  <?php endwhile; ?>
      </select>
    </div>
    <!--    Parent-->
    ...
    <div class="form-group col-md-3">
      ...
      <textarea name="shortdescription" id="shortdescription" rows="6"
                class="form-control"><?= $shortdescription; ?></textarea>
    </div>
    <!--    Add Product-->
...
<?php } else {
  ...
  <a href="products.php?add=1" class="btn btn-success pull-right" id="add-product-button" style="margin-top: -35px;">Add
    Product</a>
